...
export class CartService {
  ...
  async createCart(createCartDto: CreateCartDto, userId: string) {
    ...
    const missingProducts = cartLines.filter(
      (cartLine) =>
    ...
    if (activeCart) {
      return await this.prisma.$transaction(async (prisma) => {
        ...
        const newTotal = updatedCartLines.reduce(
  ...
  async deleteCart(cartId: string, userId: string) {
    ...
    if (!cart) {
      throw new NotFoundException(
        translate(this.i18n, 'messages.cartNotFound'),
      );
    }

    if (cart.userId !== userId) {
      throw new UnauthorizedException(
        translate(this.i18n, 'messages.unauthorized'),
      );
    }
